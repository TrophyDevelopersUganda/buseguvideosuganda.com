...
<?php else: ?>
	<h1><?php esc_html_e( 'Welcome to '.apply_filters('betheme_label', 'BuseguVideosuganda'), 'mfn-opts' ); ?></h1>
<?php endif; ?>
...
<h2 class="nav-tab-wrapper wp-clearfix">
	<a href="admin.php?page=<?php echo apply_filters('betheme_dynamic_slug', 'buseguvideosuganda'); ?>" class="nav-tab<?php if( $current_screen == 'toplevel_page_'. apply_filters('betheme_dynamic_slug', 'buseguvideosuganda') ) echo ' nav-tab-active'; ?>"><?php esc_html_e( 'Welcome', 'mfn-opts' ); ?></a>

	<?php if( ! mfn_is_hosted() ): ?>
		<a href="admin.php?page=<?php echo apply_filters('betheme_slug', 'be'); ?>-status" class="nav-tab<?php if( $current_screen == apply_filters('betheme_label', 'buseguvideosuganda').'_page_'. apply_filters('betheme_slug', 'be') .'-status' ) echo ' nav-tab-active'; ?>"><?php esc_html_e( 'System status', 'mfn-opts' ); ?></a>
	<?php endif; ?>

	<?php if( ! $is_support_disabled ): ?>
		<a href="admin.php?page=<?php echo apply_filters('betheme_slug', 'be'); ?>-support" class="nav-tab<?php if( $current_screen == apply_filters('betheme_label', 'buseguvideosuganda').'_page_'. apply_filters('betheme_slug', 'be') .'-support' ) echo ' nav-tab-active'; ?>"><?php esc_html_e( 'Manual & Support', 'mfn-opts' ); ?></a>
	<?php endif; ?>

	<?php if( ! $is_changelog_disabled ): ?>
		<a href="admin.php?page=<?php echo apply_filters('betheme_slug', 'be'); ?>-changelog" class="nav-tab<?php if( $current_screen == apply_filters('betheme_label', 'buseguvideosuganda').'_page_'. apply_filters('betheme_slug', 'be') .'-changelog' ) echo ' nav-tab-active'; ?>"><?php esc_html_e( 'Changelog', 'mfn-opts' ); ?></a>
	<?php endif; ?>
